...
SC_MODULE(tb_tensor) {
    ...
    void drive_test() {
        ...
        sc_uint<32> current_address = 0x2000; 
        int rows_written = 0;
        ...
        while (rows_written < 32) { 
            ...
            for (int i = 0; i < 4; i++) {
                WDATA.write(0xCC000000 + rows_written + i); // Tensor Signature Data
                WVALID.write(1);
        ...
        wait(100);
        current_address = 0x2000; int rows_read = 0;
        while (rows_read < 32) {
            ...
            while (read_count < 4) {
                wait(); 
                if (RVALID.read() == 1) {
                    cout << "Tensor Read Row " << rows_read << " Data: " << hex << RDATA.read() << endl;
                    read_count++;
                }
            }
    ...
    SC_CTOR(tb_tensor) { SC_THREAD(drive_test); sensitive << ACLK.pos(); }
};
#endif